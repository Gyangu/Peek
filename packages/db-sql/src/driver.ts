...
export function requireMysqlConfig(cfg: ConnectionConfig): MysqlConnectionConfig {
  if (!isDriverConfig<MysqlConnectionConfig>(cfg, 'mysql')) {
    throw peekError(
      'BAD_REQUEST',
      `db-sql received a connection config with driverId=${cfg.driverId}, expected mysql`,
    )
  }
  return cfg
}
...
export function requireSqliteConfig(cfg: ConnectionConfig): SqliteConnectionConfig {
  if (!isDriverConfig<SqliteConnectionConfig>(cfg, 'sqlite')) {
    throw peekError(
      'BAD_REQUEST',
      `db-sql received a connection config with driverId=${cfg.driverId}, expected sqlite`,
    )
  }
  return cfg
}
