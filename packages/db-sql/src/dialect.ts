...
/**
 * The SQL dialect layer — the reason `@peek/db-sql` is one package and not two.
 *
 ...
 *
 * Written out rather than `Extract<DriverId, 'mysql' | 'sqlite'>`, which is what
 * it was while `DriverId` was a union of six literals. `DriverId` is a string
 * with a shape now — a package installed at runtime brings its own id — and
 * `Extract<string, …>` is `never`, so the old spelling silently emptied the type
 * instead of narrowing it. The pair is this package's own claim in any case:
 * which databases `db-sql` ships is `db-sql`'s business.
 */
export type SqlFlavor = 'mysql' | 'sqlite'
