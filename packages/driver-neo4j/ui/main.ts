 ...
 *
 * This file is the whole of a Tier C plugin view
 * (`docs/design/2026-08-03-plugin-architecture.md` §2.6, amended by §2.6bis). It
 * runs in an iframe on `peek-plugin://neo4j`, which means:
 *
 ...
 *   arrangement `plugin-channel.ts` describes as "the frame is given types only".
...
/**
 * `PluginErrorMessageSchema` caps `message` at 2000 characters. Past that, zod
...
function send(message: PluginClientMessage): void {
 ...
 * centre is the entire reason `PluginErrorMessage` is in the protocol.
 */
...
/**
 * The handshake, in the order `plugin-channel.ts` fixes it.
 *
 ...
 *
 * The mirror of `parsePluginClientMessage` on the other side, minus zod. Written
...
function asStatus(value: unknown): PluginDataStatus {
  return typeof value === 'string' && STATUSES.includes(value) ? (value as PluginDataStatus) : 'idle'
}
 ...
 */
function readHostMessage(data: unknown): PluginHostMessage | null {
  ...
  switch (m['t']) {
    case 'init':
      return {
        ...
        pluginKind: asString(m['pluginKind'], 'graph'),
 ...
 * of the file: importing them would drag `composeGraphQuery` into the plugin
...
let status: PluginDataStatus = 'idle'
let truncated = false
let rowCount = 0
/** How many rows the last snapshot actually carried. The truncation banner needs
 *  a numerator, and `PluginDataMessage` only states the denominator. */
let shownRows = 0
 ...
 * reshuffled".
 */
function rebuild(message: PluginDataMessage): boolean {
 ...
 * markup here would run on `peek-plugin://neo4j` with the one capability that
...
/*
 * The way back out of a focus. `null` deletes the key — `PluginPatchMessage`
...
function applyTheme(theme: PluginTheme): void {
  document.documentElement.dataset['theme'] = theme
  palette = readPalette()
  draw()
}

function applyHostMessage(message: PluginHostMessage): void {
  switch (message.t) {
    case 'init': {
