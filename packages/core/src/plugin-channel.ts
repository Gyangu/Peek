...
/* ==================================================================
 * The per-view channel between the window and a self-drawn plugin view.
 *
 * A Tier C plugin (`docs/design/2026-08-03-plugin-architecture.md` §2.6) draws
 * itself inside an iframe on its own `peek-plugin://<id>` origin. That origin is
 ...
 *   `autoFetch` — so the *statement* is always composed by the plugin's
 ...
 *
 * The host validates everything arriving from the frame (`parsePluginClientMessage`),
 ...
 * plugin bundle carries no zod and shares no chunk with the window. See
 * `apps/desktop/src/plugin-ui/`.
 * ================================================================== */
 ...
 */
export const PLUGIN_MAX_ROWS = 2000

export type PluginTheme = 'light' | 'dark'

/** What the host knows about the view's fetch, in the frame's vocabulary. */
export const PluginDataStatusSchema = z.enum(['idle', 'loading', 'done', 'error'])
export type PluginDataStatus = z.infer<typeof PluginDataStatusSchema>
...
export interface PluginThemeMessage {
  t: 'theme'
  theme: PluginTheme
}
...
export interface PluginDataMessage {
  ...
  rowCount: number
  /** Set when `rows` is a prefix — either past `PLUGIN_MAX_ROWS`, or the driver truncated. */
  truncated: boolean
 ...
 */
export const PluginReadyMessageSchema = z.object({ t: z.literal('ready') })

/**
 * Change this view's own state. Nothing else.
 *
 * Shaped exactly like the `plugin` member of `ViewPatch` because that is what the
 ...
 */
export const PluginPatchMessageSchema = z.object({
  t: z.literal('patch'),
...
export const PluginClientMessageSchema = z.discriminatedUnion('t', [
  PluginReadyMessageSchema,
  PluginPatchMessageSchema,
  PluginErrorMessageSchema,
])

export type PluginClientMessage = z.infer<typeof PluginClientMessageSchema>
export type PluginPatchMessage = z.infer<typeof PluginPatchMessageSchema>
 ...
 * nonsense is a broken plugin, not a broken window, and taking the window down
 * with it would be the wrong blast radius. The host counts the drops so a plugin
 * that only ever sends nonsense is still visible.
 */
export function parsePluginClientMessage(data: unknown): PluginClientMessage | null {
  const parsed = PluginClientMessageSchema.safeParse(data)
