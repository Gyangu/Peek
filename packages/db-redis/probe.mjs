...
await c.zAdd(P + 'zset', [{ value: 'a', score: 1.5 }, { value: 'b', score: 2 }])
...
const keys = s.keys
const t0 = Date.now()
const results = await Promise.allSettled(keys.flatMap((k) => [
  c.type(k), c.pTTL(k), c.memoryUsage(k), c.objectEncoding(k),
]))
console.log('pipeline ms', Date.now() - t0, results.map(r => r.status + ':' + JSON.stringify(r.value ?? r.reason?.message)).join(' | '))
...
console.log('strLen', await c.strLen(P + 'str'), 'getRange', JSON.stringify(await c.getRange(P + 'str', 0, 4)))
...
// wrong type error
try { await c.lRange(P + 'hash', 0, 1) } catch (e) { console.log('wrongtype', e.constructor.name, e.message) }
// nonexistent
console.log('type missing', await c.type(P + 'nope'), 'pttl missing', await c.pTTL(P + 'nope'), 'memusage missing', await c.memoryUsage(P + 'nope'))
try { console.log('objenc missing', await c.objectEncoding(P + 'nope')) } catch (e) { console.log('objenc missing err', e.message) }
