...
import {
  PLUGIN_MAX_ROWS,
  parsePluginClientMessage,
  type PluginDataMessage,
  type PluginDataStatus,
...
/* ==================================================================
 * The host side of a Tier C plugin view: an iframe, and one MessagePort.
 *
 ...
 * itself is `@peek/core/plugin-channel`, and it is the whole of what may cross.
 *
 ...
 * frame goes through `parsePluginClientMessage` before it is looked at, and the
 ...
 * never send `ready`, it may send nonsense forever. Those are a plugin being
 * broken, not a plugin being dangerous, and the answers to them are the frame
 ...
 * therefore remove its own restrictions — and a plugin needs both. Writing the
 ...
 * response CSP in `main/plugins/protocol.ts`, both of which are outside the
 * frame's reach. Same conclusion VS Code reached for webviews.
 * ================================================================== */

export interface PluginFrameProps {
  view: PluginViewState
  /** The plugin whose UI to load; its origin is `peek-plugin://<pluginId>`. */
  pluginId: string
}
 ...
 *
 * Sent anyway, because the protocol declares the field and a plugin that reads
 * it must get a real answer rather than `undefined` — and because the day peek
 * grows a light theme, the plugins that were written against this are the ones
 * that follow along for free.
 */
const THEME: PluginTheme = 'dark'
 ...
 * costs is how quickly a genuinely broken plugin is named. Short enough to fire
 * before anyone starts wondering, long enough that a machine under load never
 * sees it.
 */
const READY_TIMEOUT_MS = 3000

export function PluginFrame({ view, pluginId }: PluginFrameProps): ReactElement {
  const t = useT()
  ...
  const origin = `peek-plugin://${pluginId}`
   ...
   * the `PLUGIN_MAX_ROWS` cap and rows that have not landed yet.
   */
  const buildData = useCallback((): PluginDataMessage => {
    const status: PluginDataStatus =
      snapshot.error !== null
        ? 'error'
    ...
    const rows: unknown[][] = []
    const ceiling = Math.min(snapshot.rowCount, PLUGIN_MAX_ROWS)
   ...
   * handshake on every patch — that is, on every interaction the plugin's own UI
   ...
   * `plugin-channel.ts`: `load` is the earliest moment the frame is guaranteed
  ...
  useEffect(() => {
    ...
    const handleLoad = (): void => {
      ...
      channel.port1.onmessage = (event: MessageEvent): void => {
        const message = parsePluginClientMessage(event.data)
        // Dropped, not thrown, and not surfaced: a frame that sends nonsense is
        // a broken plugin, and taking the window down — or filling the error
        ...
        switch (message.t) {
          case 'ready':
            ...
            post({
              ...
              pluginKind: current.view.pluginKind,
            ...
            return
          case 'patch':
            // The same Command the built-in controls send. A plugin therefore
            ...
            return
          case 'error':
            // Surfaced, unlike a malformed message: this one the plugin author
            // chose to send, and a frame with no console anyone opens has no
            // other way to be heard.
            notify(
              'error',
              current.t('view.pluginError', { kind: current.view.pluginKind }),
              message.message,
            )
            return
        }
      ...
      frame.contentWindow?.postMessage({ t: 'peek-plugin-port' }, origin, [channel.port2])
    }
   ...
   * never built (`out/plugin-ui/<id>/index.html` is a 404, and the frame that
   * loads instead has no script to answer with), or the plugin's own boot threw
   ...
   * what this view can consume, and `atBottom: false` because a plugin view is
   * never asking for more than its cap.
   */
  useEffect(() => {
    const resultId = view.resultId
    if (resultId === undefined) return
    setViewport(resultId, 0, PLUGIN_MAX_ROWS - 1, false)
    return () => {
