import type { DriverId, PluginViewState, ViewKindLookup, ViewKindRegistration } from '@peek/core'
 ...
 * because the two halves arrive from different files in a plugin package (a
...
export interface RendererViewKind {
   ...
   * one argument without a cast — and it means a plugin naming a key the catalog
   * does not have fails here rather than painting the raw key into the tab strip.
   */
  titleKey: PlainMessageKey
  render(view: PluginViewState): ReactElement
}
...
/**
 * Register one plugin view kind, or refuse it and say why.
 *
 * **Refusing is the design.** A plugin kind cannot have the compile-time
 ...
 * from several plugins wants to report all the bad ones, not die on the first.
 */
export function registerViewKind(entry: RendererViewKind): RegisterViewKindOutcome {
  ...
  if (REGISTRY.has(kind)) {
    // Two plugins claiming one kind is not resolvable by policy — whichever won
    // would draw the other's data. Refusing the second keeps the first working.
    return { ok: false, reason: `view kind "${kind}" is already registered by another plugin` }
  }
  REGISTRY.set(kind, entry)
  return { ok: true }
}

/** Drop a kind, e.g. when its plugin is uninstalled. Views of that kind then render as unavailable. */
 ...
 */
export const viewKindLookup: ViewKindLookup = (pluginKind) => lookupViewKind(pluginKind)?.contract ?? null
