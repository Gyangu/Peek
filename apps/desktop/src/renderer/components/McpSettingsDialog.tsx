 ...
 */
export function McpSettingsDialog({ onClose }: { onClose: () => void }): ReactElement {
  const t = useT()
  ...
  // wrong. One second is slow enough to be free and fast enough that the user
  ...
  return (
    <div className="modal-mask" onMouseDown={onClose}>
      <div className="modal" style={{ width: 620 }} onMouseDown={stop}>
        <div className="modal-head">
          <span className="t">{t('mcp.title')}</span>
          <span style={{ flex: 1 }} />
          <button className="ghost" onClick={onClose} aria-label={t('mcp.close')}>
            ✕
          </button>
        </div>
