 ...
 * enough.
 *
 * `manifest-purity.test.ts` protects `@peek/db-x/manifest` because the
 * renderer loads it and a leak there puts `pg` in the window. Three more
 * subpaths have appeared since, and they are loaded by **main** or by the
...
/** `…/apps/desktop/src/main/packages/__tests__` → the workspace root. */
...
const SUBPATHS: readonly { pkg: string; subpath: string; file: string; why: string }[] = [
  {
    ...
    subpath: './view',
    file: 'src/view.ts',
    why: "main plans a package view's autoFetch from inside a Command reduction",
  },
  {
    pkg: 'db-neo4j',
  ...
  },
  {
    ...
    why: 'the mappings behind them run in the package host, which is not a driver host either',
  },
...
describe('client-free subpaths loaded by main', () => {
  ...
  test('each one is aliased in the vite config, or the build resolves it into index.ts', () => {
    ...
    // entry, `@peek/db-neo4j/mcp-tools` matches the bare `@peek/db-neo4j`
