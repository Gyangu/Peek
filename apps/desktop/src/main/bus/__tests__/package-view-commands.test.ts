...
/* ==================================================================
 * `view.open` and `view.update` carry a package view.
 *
 * These two commands are the *only* part of the Command contract that had to
 * open for packages. All 32 names in `COMMAND_NAMES` are kernel-generic —
 * connections, layout, chat, settings — so a package needs no new verb, only for
 ...
 * `applyViewPatch`'s `case 'package'` and none of them is forced by the types,
...
async function openPackageView(
  ...
  const res = await h.bus.dispatch(
    'view.open',
    { spec: { kind: 'package', packageKind: 'documents', connId, state } },
    'ui',
  )
  assert.equal(res.ok, true, 'a package spec must be accepted by view.open')
  if (!res.ok) throw new Error('unreachable')
  return res.data.viewId
}

function packageViewOf(h: Harness, viewId: ViewId): PackageViewState {
  const view = h.store.getState().views[viewId]
  assert.ok(view, 'the view must exist')
  assert.equal(view.kind, 'package')
  if (view.kind !== 'package') throw new Error('unreachable')
  return view
}

describe('view.open accepts a package view', () => {
  it('stores the package kind and its state verbatim', async () => {
    const h = harness()
    const connId = await connect(h)
    const viewId = await openPackageView(h, connId)

    const view = packageViewOf(h, viewId)
    assert.equal(view.packageKind, 'documents')
    // Verbatim: the kernel does not know this shape, so it must not normalise,
    // reorder or drop anything in it. The package's own schema is the only thing
  ...
  it('a spec with no state opens with an empty one rather than undefined', async () => {
    ...
    const res = await h.bus.dispatch(
      'view.open',
      { spec: { kind: 'package', packageKind: 'graph', connId } },
      'ui',
    )
    assert.equal(res.ok, true)
    if (!res.ok) throw new Error('unreachable')
    assert.deepEqual(packageViewOf(h, res.data.viewId).state, {})
  })

  it('rejects a spec with no packageKind', async () => {
    const h = harness()
    const connId = await connect(h)
    const res = await h.bus.dispatch(
      'view.open',
      { spec: { kind: 'package', packageKind: '', connId } },
      'ui',
    )
    assert.equal(res.ok, false, 'an empty packageKind names no registered kind and cannot be opened')
  })
})

describe('view.update merges a package view’s state', () => {
  it('merges rather than replaces — the built-in patches all behave this way', async () => {
    ...
    // package patch that replaced the whole state would behave differently for no
    ...
    const h = harness()
    const connId = await connect(h)
    const viewId = await openPackageView(h, connId, { collection: 'orders', limit: 100 })

    const res = await h.bus.dispatch(
      'view.update',
      { viewId, patch: { kind: 'package', state: { limit: 500 } } },
      'ui',
    )
    assert.equal(res.ok, true)
    assert.deepEqual(packageViewOf(h, viewId).state, { collection: 'orders', limit: 500 })
  })
  ...
  it('null deletes a key, which is the only way a patch can express removal', async () => {
    ...
    const h = harness()
    const connId = await connect(h)
    const viewId = await openPackageView(h, connId, { collection: 'orders', filter: 'x>1' })

    const res = await h.bus.dispatch(
      'view.update',
      { viewId, patch: { kind: 'package', state: { filter: null } } },
      'ui',
    )
    assert.equal(res.ok, true)
    assert.deepEqual(packageViewOf(h, viewId).state, { collection: 'orders' })
  })

  it('deleting a key that was not there changes nothing and is not an error', async () => {
    const h = harness()
    const connId = await connect(h)
    const viewId = await openPackageView(h, connId, { collection: 'orders' })

    const res = await h.bus.dispatch(
      'view.update',
      { viewId, patch: { kind: 'package', state: { absent: null } } },
      'ui',
    )
    assert.equal(res.ok, true)
    assert.deepEqual(packageViewOf(h, viewId).state, { collection: 'orders' })
  ...
  it('refuses a patch whose kind does not match the view', async () => {
    ...
    // from being applied to a package view, and vice versa.
    const h = harness()
    const connId = await connect(h)
    const viewId = await openPackageView(h, connId)
